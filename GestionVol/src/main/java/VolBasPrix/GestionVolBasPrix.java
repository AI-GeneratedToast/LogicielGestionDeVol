...
public class GestionVolBasPrix extends JFrame {
    ...
    public GestionVolBasPrix()throws Exception {
        initUI();
    }
    ...
    private void initUI() throws Exception {
        ...
        ResultSet resultSetType = ControlleurVolBasPrix.getInstance().lister();
        ...
        while (resultSet.next() && resultSetType.next()) {
            ...
            int pk = resultSetType.getInt("idvolbasprix");
            ...
            VolBasPrix vol = new VolBasPrix(fk, numvol, destination, datedepart,nbreservations,idavion,repas,prise,bar,
                    servpayant,wifi,servdivertissement,reservSiege,pk);

            model.addRow(new Object[] { vol.getIdVolBasPrix(), vol.getNumVol(), vol.getDestination(), vol.getIdAvion(),
    ...
    private void ajouter() throws Exception {
        ...
        VolBasPrix vol = new VolBasPrix(fk,numVol,dest,depart,reserv,idAvion,repas,prise,bar,servpayant,wifi,
        ...
        vol.setIdVol(fk);
        ControlleurVolBasPrix.getInstance().enregistrer(vol);


        //Prendre le pk de la bd
        ResultSet resultSetVolBasPrix = ControlleurVolBasPrix.getInstance().lister();
        resultSetVolBasPrix.last();
        pk = resultSetVolBasPrix.getInt("idvolbasprix");
        vol.setIdVolBasPrix(pk);
    ...
    private void modifier() throws Exception {
        ...
        resultSetType = ControlleurVolBasPrix.getInstance().lister();
        ...
        ControlleurVolBasPrix.getInstance().modifier(vol);
    ...
    private void supprimer() throws Exception {
        ...
        //Se Positioner dans le champ choisit
        rowIndex = tableVol.getSelectedRow()+1;//+1 parce que result set doit commencer à 1
        resultSetVolType = ControlleurVolBasPrix.getInstance().lister();
        ...
        idVolBasPrix = resultSetVolType.getInt("idvolbasprix");
        idVol = resultSetVol.getInt("idvol");

        //Supprime le vol dans la liste
        ControlleurVolBasPrix.getInstance().supprimer(idVolBasPrix);
