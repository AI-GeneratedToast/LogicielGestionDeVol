...
public class GestionVolPrive extends JFrame {
    ...
    public GestionVolPrive()throws Exception {
        initUI();
    }
    ...
    private void initUI() throws Exception {
        ...
        ResultSet resultSetType = ControlleurVolPrive.getInstance().lister();
        ...
        while (resultSet.next() && resultSetType.next()) {
            ...
            int pk = resultSetType.getInt("idvolprive");
            ...
            VolPrive vol = new VolPrive(pk, numvol, destination, datedepart,nbreservations,idavion,repas,prise,bar,
    ...
    private void ajouter() throws Exception {
        ...
        VolPrive vol = new VolPrive(fk,numVol,dest,depart,reserv,idAvion,repas,prise,bar,servpayant,wifi,
        ...
        vol.setIdVol(fk);
        ControlleurVolPrive.getInstance().enregistrer(vol);


        //Prendre le pk de la bd
        ResultSet resultSetVolBasPrix = ControlleurVolPrive.getInstance().lister();
        resultSetVolBasPrix.last();
        pk = resultSetVolBasPrix.getInt("idvolprive");
        vol.setIdVolType(pk);
    ...
    private void modifier() throws Exception {
        ...
        resultSetType = ControlleurVolPrive.getInstance().lister();
        ...
        VolPrive vol = new VolPrive(fk, numvol, destination, datedepart,nbreservations,idavion,repas,prise,bar,
                servpayant,wifi,servdivertissement,reservSiege,pk);

        ControlleurVol.getInstance().modifier(vol);
        ControlleurVolPrive.getInstance().modifier(vol);
    ...
    private void supprimer() throws Exception {
        ...
        //Se Positioner dans le champ choisit
        rowIndex = tableVol.getSelectedRow()+1;//+1 parce que result set doit commencer à 1
        resultSetVolType = ControlleurVolPrive.getInstance().lister();
        ...
        idVolType = resultSetVolType.getInt("idvolprive");
        idVol = resultSetVol.getInt("idvol");

        //Supprime le vol dans la liste
        ControlleurVolPrive.getInstance().supprimer(idVolType);
