...
public class GestionVolRegulier extends JFrame {
    ...
    public GestionVolRegulier()throws Exception {
        initUI();
    }
    ...
    private void initUI() throws Exception {
        ...
        ResultSet resultSetType = ControlleurVolRegulier.getInstance().lister();
        ...
        while (resultSet.next() && resultSetType.next()) {
            ...
            int pk = resultSetType.getInt("idvolregulier");
            ...
            VolRegulier vol = new VolRegulier(pk, numvol, destination, datedepart,nbreservations,idavion,repas,prise,bar,
    ...
    private void ajouter() throws Exception {
        ...
        VolRegulier vol = new VolRegulier(fk,numVol,dest,depart,reserv,idAvion,repas,prise,bar,servpayant,wifi,
        ...
        vol.setIdVol(fk);
        ControlleurVolRegulier.getInstance().enregistrer(vol);


        //Prendre le pk de la bd
        ResultSet resultSetVolBasPrix = ControlleurVolRegulier.getInstance().lister();
        resultSetVolBasPrix.last();
        pk = resultSetVolBasPrix.getInt("idvolregulier");
        vol.setIdVolType(pk);
    ...
    private void modifier() throws Exception {
        ...
        resultSetType = ControlleurVolRegulier.getInstance().lister();
        ...
        VolRegulier vol = new VolRegulier(fk, numvol, destination, datedepart,nbreservations,idavion,repas,prise,bar,
                servpayant,wifi,servdivertissement,reservSiege,pk);

        ControlleurVol.getInstance().modifier(vol);
        ControlleurVolRegulier.getInstance().modifier(vol);
    ...
    private void supprimer() throws Exception {
        ...
        //Se Positioner dans le champ choisit
        rowIndex = tableVol.getSelectedRow()+1;//+1 parce que result set doit commencer à 1
        resultSetVolType = ControlleurVolRegulier.getInstance().lister();
        ...
        idVolType = resultSetVolType.getInt("idvolregulier");
        idVol = resultSetVol.getInt("idvol");

        //Supprime le vol dans la liste
        ControlleurVolRegulier.getInstance().supprimer(idVolType);
