...
public class DAOVolRegulier {
    ...
    // ****************** PARTIE SINGLETON *********************
    private static DAOVolRegulier instance = null;

    private DAOVolRegulier(){}
    public static DAOVolRegulier getInstance(){
        if (instance == null){
            instance =  new DAOVolRegulier();
        }
        return instance;
    }
    //************************ FIN PARTIE SINGLETON ********************
    public String enregistrerVolRegulier(VolRegulier vol) throws Exception {
        String msg ="";

        try{
            connection = ConnexionDB.getInstance().getConnexion();
            if (connection!=null){
                preparedStatement = connection.prepareStatement("INSERT INTO volregulier (isRepasFourni, isSiegeReserve, " +
    ...
    public ResultSet listerVolRegulier() throws Exception {

        try{
            connection = ConnexionDB.getInstance().getConnexion();
            if (connection!= null){
                statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                resultSet = statement.executeQuery("SELECT * FROM volregulier");
    ...
    public String supprimerVolRegulier(int id) throws Exception {
        String msg ="";

        try{
            connection = ConnexionDB.getInstance().getConnexion();
            if (connection!=null){
                preparedStatement = connection.prepareStatement("DELETE FROM volregulier WHERE idvolregulier=?");
    ...
    public Object listeUnVolRegulier(int id) throws Exception {
        String msg ="";

        try{
            connection = ConnexionDB.getInstance().getConnexion();
            if (connection != null){
                preparedStatement = connection.prepareStatement("SELECT * FROM volregulier WHERE idvolregulier=?");
                preparedStatement.setInt(1,id);
    ...
    public String modifierVolRegulier(VolRegulier vol){
        String msg ="";

        try{
            connection = ConnexionDB.getInstance().getConnexion();
            if ( connection != null){
                preparedStatement = connection.prepareStatement("UPDATE volregulier SET isrepasfourni=?," +
                        "issiegereserve=?, isservicebar=?, issysdivertissement=?, isservicepayant=?," +
                        "isprisealimentation=?, iswifi=?, idvol=? WHERE idvolregulier=?");
                preparedStatement.setBoolean(1,vol.getIsRepasFourni());
                preparedStatement.setBoolean(2,vol.getIsSiegeReserve());
                preparedStatement.setBoolean(3,vol.getIsServiceBar());
                preparedStatement.setBoolean(4,vol.getIsSysDivertissement());
